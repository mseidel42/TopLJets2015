...
def main():
    ...
    parser.add_option('-j', '--json',        dest='json'  ,      help='json with list of files',        default='data/era2016/samples.json,data/era2016/qcd_samples.json',              type='string')
    ...
    weightVars = 'pu_down,pu_up,leptrig_up,leptrig_down,lepsel_up,lepsel_down,bfrag_up,bfrag_down,bfrag_peterson,slbr_up,slbr_down,top_pt,id1005muR2muF2hdampmt272.7225,id1009muR0.5muF0.5hdampmt272.7225,id3001PDFset13100,id4001PDFset25200'.split(',')
    ...
    # pass through alphas CMW-2loop samples
    ...
    for alphas in cmw_as:
        for scale in cmw_scale:
            varList.append('pythia8_asfsr%s_scale%s_CMW_2loop'%(alphas, scale))
    ...
    for slist,isSyst in [ (samplesList,False), (systSamplesList,True), (expSystSamplesList,True) ]:
        for tag,sample in slist:
            ...
            if isSig:
                ...
                for w in weightVars:
                    print(opt.obs+'_'+opt.reco+'_'+opt.flavor+'_'+w+'_responsematrix')
                    ih=fIn.Get(opt.obs+'_'+opt.reco+'_'+opt.flavor+'_'+w+'_responsematrix')
    ...
    background_systematics = [
        ...
        [
            ['qcd'],
            ['MC13TeV_QCDMuEnriched30to50', 1.],
            ['MC13TeV_QCDMuEnriched50to80', 1.],
            ['MC13TeV_QCDMuEnriched80to120', 1.],
            ['MC13TeV_QCDMuEnriched120to170', 1.],
            ['MC13TeV_QCDMuEnriched170to300', 1.],
            ['MC13TeV_QCDMuEnriched300to470', 1.],
            ['MC13TeV_QCDMuEnriched470to600', 1.],
            ['MC13TeV_QCDMuEnriched600to800', 1.],
            ['MC13TeV_QCDMuEnriched800to1000', 1.],
            ['MC13TeV_QCDMuEnriched1000toInf', 1.],
            ['MC13TeV_QCDEMEnriched30to50', 1.],
            ['MC13TeV_QCDEMEnriched50to80', 1.],
            ['MC13TeV_QCDEMEnriched80to120', 1.],
            ['MC13TeV_QCDEMEnriched120to170', 1.],
            ['MC13TeV_QCDEMEnriched170to300', 1.],
            ['MC13TeV_QCDEMEnriched300toInf', 1.],
        ]
    ]
    ...
    dataUnfoldedSysRatio.GetXaxis().SetTitleSize(0.2*p2correction)
    dataUnfoldedSysRatio.GetXaxis().SetTitleOffset(0.95)
    dataUnfoldedSysRatio.GetXaxis().SetLabelSize(0.18*p2correction)
    ...
    a14Gen = normalizeAndDivideByBinWidth(systematics['MC13TeV_TTJets_pythia8a14'].ProjectionX("a14Gen"))
    for i in range(1, a14Gen.GetNbinsX()+1):
        a14Gen.SetBinError(i, 1e-20)
    a14Gen.SetLineColor(ROOT.kOrange+2)
    a14Gen.SetLineStyle(1)
    a14Gen.SetLineWidth(2)
    a14Gen.SetMarkerColor(ROOT.kOrange+2)
    a14Gen.SetMarkerStyle(27)
    #a14Gen.Draw('SAME H')
    a14GenRatio=a14Gen.Clone('a14GenRatio')
    a14GenRatio.Divide(dataUnfoldedNoErr)
    ...
    legend = ROOT.TLegend(inix,0.45,inix+0.4,0.9)
    ...
    txt.SetNDC(True)
    txt.SetTextFont(42)
    txt.SetTextSize(0.05*p1correction)
    txt.SetTextAlign(12)
    ...
    dataUnfoldedSysRatio.SetLineWidth(0)
    dataUnfoldedSysRatio.SetLineWidth(0)
    dataUnfoldedSysRatio.Draw('e2')
    legend.AddEntry(dataUnfoldedSysRatio, 'Total uncertainty', 'f')
    
    dataUnfoldedRatio.SetLineWidth(0)
    dataUnfoldedRatio.SetFillColor(ROOT.kBlack)
    dataUnfoldedRatio.Draw('e2,same')
    legend.AddEntry(dataUnfoldedRatio, 'Statistical uncertainty', 'f')
    
    syslist = [['MC13TeV_TTJets_tracking_up', 'Tracking up', ROOT.kCyan+1, 22],
              ['MC13TeV_TTJets_tracking_down', 'Tracking down', ROOT.kCyan+1, 23],
              ['MC13TeV_TTJets_fsrup', 'FSR up (uncertainty)', ROOT.kRed+1, 26],
              ['MC13TeV_TTJets_fsrdn', 'FSR down (uncertainty)', ROOT.kRed+1, 32],
    ...
    for sys in syslist: #systematicUnfolded.iteritems():
        ...
        if not 'herwig' in sys[0]: legend.AddEntry(hist, sys[1], 'p')
    ...
    legend.AddEntry(FSRUpGenSens, 'FSR up (full effect)', 'pl')
    ...
    legend.AddEntry(FSRDownGenSens, 'FSR down (full effect)', 'pl')
    herwigGenSens = herwigGen.Clone('herwigGenSens')
    herwigGenSens.Divide(nominalGen)
    herwigGenSens.SetLineWidth(2)
    herwigGenSens.Draw('same h')
    legend.AddEntry(herwigGenSens, 'HERWIG++ (uncertainty)', 'p')
    legend.AddEntry(herwigGenSens, 'HERWIG++ (full effect)', 'pl')
    ...
    txt.SetTextAlign(12)
    txt.DrawLatex(0.16,0.91,'#scale[1.2]{%s #it{Simulation}}'%cmsLabel)
