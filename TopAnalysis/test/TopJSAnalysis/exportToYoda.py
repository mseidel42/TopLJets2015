...
fOut={'data' : open(os.path.join(outDir,name+'_nonHEPData.yoda'),'w'),
      'mc'   : open(os.path.join(outDir,name+'_MC.yoda'),'w'),
      'plot' : open(os.path.join(outDir,name+'.plot'),'w')}


fOut['plot'].write('''# BEGIN PLOT /CMS_2018_I1690148/*
LogY=0
LogX=0
XTwosidedTicks=1
YTwosidedTicks=1
...
for reco in recos:
    for obs in observables:
        ...
        for flavor in flavors:
            iflavor += 1
            pname = 'd{0:0>2}-x01-y{1:0>2}'.format(iobs,iflavor)
            ...
            fOut['plot'].write('''
# BEGIN PLOT /CMS_2018_I1690148/%s
